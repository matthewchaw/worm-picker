...
class Page3(ttk.Frame):
	...
	def __init__(self, parent, controller):
		...
		self.pickBox.pack(fill = 'both', expand = True, side = 'left')
		self.pickBox.grid_rowconfigure(0, weight=1)
		self.pickBox.grid_columnconfigure(0, weight=1)

		self.rightFrame = ttk.Frame(self.toDoWin)
		self.rightFrame.pack(fill = 'both', expand = True, side = 'right')

		self.eventLogFrame = ttk.LabelFrame(self.rightFrame, text = 'Event Log')#, background='cyan')
		self.eventLogFrame.grid(row = 0, padx=5, pady = 5,sticky = 'nesw')
		

		self.eventLog = tk.Text(self.eventLogFrame)#) #Inside frame
		self.eventLog.grid(row = 0, padx=5, pady = 5,sticky = 'nesw')
...
class WeeklyPick(ttk.Frame):
	def __init__(self, parent, controller):
		...
		self.weeklyPickFrame = ttk.LabelFrame(self.parent, text = 'Weekly Outlook')
		...
		for i in range(todaywd,todaywd+7): #Creates respective list starting at today, ending 6 days from now
			if (i+1) >7:
				weekdayidx.append((i - 7))
			else:
				weekdayidx.append(i)

	def getDayCounts(self):
		self.maxDay = datetime.date.today() + datetime.timedelta(weeks=1)
...
class StrainsToPick(ttk.Frame):
	...
	def __init__(self, parent, controller):
		...
		self.mfdesc = ttk.Label(self.descbox, textvariable = self.mfmsg)
